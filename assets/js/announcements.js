...
document.addEventListener("DOMContentLoaded", function () {
  ...
  gsap.utils.toArray(".announcement").forEach((announcement, i) => {
  ...
  // --- Dress Code Card Flip Logic ---
  const cards = document.querySelectorAll(".dress-code-card");
  cards.forEach((card) => {
  ...
  // --- Dress Code Card Animations ---
  gsap.utils.toArray(".dress-code-card").forEach((card, i) => {
    gsap.from(card, {
      scrollTrigger: {
        trigger: card,
        start: "top 85%",
      ...
      duration: 0.7,
      delay: i * 0.2,
      clearProps: "transform", // Ensures GSAP releases control for the CSS flip to work
    });
  });
});
      ...
      function slideImgUpdate() {
        slides.forEach((slide) => {
          ...
          const prog = gsap.utils.mapRange(
            -rect.width,
            innerWidth,
            0,
            1,
